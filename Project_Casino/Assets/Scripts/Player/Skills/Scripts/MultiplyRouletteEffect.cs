...
[CreateAssetMenu(menuName = "Cards/Effects/Multiply Roulette")]
public class MultiplyRouletteEffect : CardEffect
{
    ...
    )
    {
        ...
        // 안전 체크
        if (RouletteUI.Instance == null)
        {
            Debug.LogError("RouletteUI Instance 없음!");
        ...
        {
            Debug.LogError("CoroutineRunner Instance 없음!");
            onFinished?.Invoke(false);
            yield break;
        }
        ...
        // 룰렛 실행
        yield return RouletteUI.Instance.Spin((result) =>
        {
            multiplier = result;
        });
